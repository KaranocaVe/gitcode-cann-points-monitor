...
After an update, refresh the GitCode tab. If Tampermonkey lists more than one copy of this script, disable or remove the older duplicate so it does not make its own requests.
...
The script runs on any GitCode page so it can display the stored total in the header and read the live CANN balance using the current logged-in browser session. It uses the same page-context request style as GitCode itself, including the browser’s origin, cookies, and access token. It checks at most once per 30-minute default interval; repeated page visits do not repeatedly call the points API. It does not run while the browser is closed.
...
- If GitCode is logged out, the API returns `401` and the previous cached value is retained until you sign in again.
